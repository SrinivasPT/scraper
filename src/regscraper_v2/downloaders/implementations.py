"""HTTP-based downloader implementations."""

import httpx
from tenacity import retry, stop_after_attempt, wait_exponential

from ..interfaces import Downloader, DownloadResult
from ..infrastructure.compliance import ThrottledRobotsChecker
        ...
        try:
            async with httpx.AsyncClient(
                ...
                return DownloadResult(
                    content=response.content, content_type=content_type, url=url
                )
...
class PlaywrightDownloader(Downloader):
    ...
    @retry(
        stop=stop_after_attempt(2), wait=wait_exponential(multiplier=1, min=3, max=30)
    )
    async def download(self, url: str) -> DownloadResult:
        ...
        try:
            async with async_playwright() as p:
                ...
                try:
                    await page.goto(
                        url, wait_until="networkidle", timeout=self._timeout
                    )
